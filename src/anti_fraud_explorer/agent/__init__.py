"""Intent router and task dispatcher for the anti-fraud RAG agent."""
...
class Agent:
    ...
    def _search_initial_candidates(
        ...
        note = (
            ...
            "请输出 action=\"search\"，并在 search_queries 列表中给出要精查的案例标题或主题；"
            "本轮最多只能连续查询两次。"
        )
    ...
    def _subsequent_fallback_result(
        ...
        if display_items:
            ...
            answer = (
                "这轮我没有拿到足够可靠的资料来回答。可以换成更具体的案例名、骗局类型或地区再问一次。"
            )
    ...
    def _handle_recommend(self, analysis) -> AgentResult:
        ...
        parts.append("")
        parts.append(
            f"下面推荐 {len(top)} 个更适合「{scene_desc}」的反诈案例。"
            f"我优先看它们是否便于{audience_desc}理解、是否具备清晰的风险信号，"
            "以及资料里能否支撑具体的提醒重点。"
        )
        parts.append("")
        ...
        for i, item in enumerate(top, 1):
            ...
            parts.append(
                ...
                f"它适合放在「{scene_desc}」里，是因为资料中明确呈现了“{display}”等风险触点，"
        ...
        return AgentResult(
            ...
            mode="local",
            confidence=0.7,
            warnings=[] if top else [f"未找到适合「{scene_desc}」的案例，建议放宽条件"],
        )
